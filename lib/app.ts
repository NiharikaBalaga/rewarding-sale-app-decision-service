...
async function bootstrap() {
  ...
  const requiredEnvVariables = [
    'MONGODB_URI_DECISION',
    'MONGO_DECISION_DATABASE',
    'JWT_ACCESS_SECRET',
  ...
  apiRouter.use('/decision', routes); // Prefixing routes with '/post'
  app.use('/api', apiRouter); // Prefixing all routes with '/api'


  try {
    await mongoose.connect(process.env.MONGODB_URI_DECISION || '', {
      dbName: process.env.MONGO_DECISION_DATABASE,
    });
